...
import { LayoutDashboard, Receipt, Users, ArrowLeftRight, User, LogOut } from 'lucide-react';
    ...
    <div className="flex min-h-screen bg-slate-950">
      ...
      <aside className="hidden md:flex w-56 flex-col border-r border-slate-800 bg-slate-950 fixed inset-y-0 left-0 z-30">
        <div className="flex items-center gap-2 px-4 py-5 border-b border-slate-800">
          ...
          <span className="font-semibold text-slate-100 text-sm">ManageCost</span>
        </div>
        ...
        <nav className="flex-1 px-2 py-4 flex flex-col gap-1">
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} className={({ isActive }) => cn(
              ...
              isActive ? 'bg-slate-800 text-slate-100' : 'text-slate-500 hover:text-slate-100 hover:bg-slate-800/50'
            )}>
        ...
        <div className="p-3 border-t border-slate-800">
          ...
          <button onClick={() => doLogout()} className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm text-slate-500 hover:text-red-400 hover:bg-slate-800/50 transition-colors">
